...
export class AircraftAPIService {
    ...
    loadRequestCount() {
        try {
            ...
            if (savedDate === today && saved) {
                ...
                console.log(`Loaded request count: ${this.requestCount}/${this.rateLimit}`);
            } else {
    ...
    saveRequestCount() {
        try {
            localStorage.setItem('aircraftAPI_requestCount', this.requestCount.toString());
        ...
        if (!this.canMakeRequest()) {
            ...
            throw new Error(
                this.dailyLimitReached
                    ...
                    : `Rate limited. Next request allowed in ${Math.ceil(timeRemaining / 1000)} seconds.`
            );
        }
    ...
    parseResponse(response) {
        ...
        for (const stateData of response.states) {
            try {
                ...
                if (aircraftState.lastContact && (now - aircraftState.lastContact) > 300) {
                    continue;
                }
        ...
        console.log(`Parsed ${aircraft.length} valid aircraft from ${response.states.length} states`);
        return aircraft;
    }
