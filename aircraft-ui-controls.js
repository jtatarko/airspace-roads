...
export class AircraftUIControls {
            ...
            <div class="aircraft-control-content">
                <div class="control-section">
                    <div class="aircraft-actions">
                        <button id="startTracking" class="action-btn primary">Start Tracking</button>
                ...
                <div class="control-section">
                    <label class="control-label">
                        <input type="checkbox" id="showAircraftLabels" ${AircraftConfig.showLabels ? 'checked' : ''}>
                        Show Aircraft Labels
                    </label>
                    <label class="control-label">
                        <input type="checkbox" id="showAircraftTrails" ${AircraftConfig.showTrails ? 'checked' : ''}>
                        Show Flight Trails
                    </label>
                </div>
    ...
    setupEventListeners() {
        ...
        // Panel toggles
        this.elements.toggleBtn.addEventListener('click', () => this.toggleMainPanel());
    ...
    setupFilterListeners() {
        ...
        // Type filters
        const typeCheckboxes = filterContainer.querySelectorAll('.type-filters input[type="checkbox"]');
     ...
     */
    updateTypeFilters() {
        const checkboxes = this.filterPanel.querySelectorAll('.type-filters input[type="checkbox"]');
     ...
     */
    updateRangeFilters() {
        const minAlt = parseInt(this.filterPanel.querySelector('#minAltitude').value) || 0;
        const maxAlt = parseInt(this.filterPanel.querySelector('#maxAltitude').value) || 50000;
        const minSpeed = parseInt(this.filterPanel.querySelector('#minSpeed').value) || 0;
        const maxSpeed = parseInt(this.filterPanel.querySelector('#maxSpeed').value) || 1000;
    ...
    async testAPI() {
        try {
            ...
            if (result.success) {
                console.log(`API test successful: ${result.aircraftCount} aircraft found`);
            ...
            if (status.status === 'running') {
                ...
                setTimeout(() => {
                    ...
                    if (currentStats.tracking.isRunning) {
                        statusElement.textContent = currentStats.tracking.isPaused ? 'Paused' : 'Running';
                        statusElement.className = `status-value ${currentStats.tracking.isPaused ? 'paused' : 'running'}`;
                    } else {
    ...
    updateStatistics(stats) {
        ...
        if (elements.totalAircraft) elements.totalAircraft.textContent = stats.total;
        if (elements.visibleAircraft) elements.visibleAircraft.textContent = stats.visible;
        if (elements.inFlightAircraft) elements.inFlightAircraft.textContent = stats.inFlight;
        if (elements.onGroundAircraft) elements.onGroundAircraft.textContent = stats.onGround;

        if (elements.avgAltitude && stats.altitudeStats) {
            elements.avgAltitude.textContent = stats.altitudeStats.average > 0
                ? `${stats.altitudeStats.average}ft`
                : '--';
        }

        if (elements.avgSpeed && stats.speedStats) {
            elements.avgSpeed.textContent = stats.speedStats.average > 0
                ? `${stats.speedStats.average}kts`
                : '--';
        }
        ...
        const infoHTML = `
            <div class="aircraft-details">
                ...
                <div class="detail-section">
                    <strong>Aircraft Type:</strong> ${aircraft.aircraftType.name}
                    <br>
                ...
                <div class="detail-actions">
                    <button onclick="aircraftControls.focusOnAircraft('${aircraft.icao24}')">
                        Follow Aircraft
                    </button>
                </div>
            </div>
    ...
    hideAircraftInfo() {
        ...
        if (this.selectedAircraft) {
            this.tracker.visualizer.selectAircraft(this.selectedAircraft.icao24, false);
            this.selectedAircraft = null;
        }
    }
