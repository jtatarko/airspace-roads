...
export class ProcessedAircraft extends AircraftState {
    constructor(aircraftState, category = null) {
        ...
        if (aircraftState instanceof AircraftState) {
            ...
            const rawData = [
                aircraftState.icao24, aircraftState.callsign, aircraftState.originCountry,
                aircraftState.timePosition, aircraftState.lastContact, aircraftState.longitude,
                aircraftState.latitude, aircraftState.baroAltitude, aircraftState.onGround,
                aircraftState.velocity, aircraftState.trueTrack, aircraftState.verticalRate,
                aircraftState.sensors, aircraftState.geoAltitude, aircraftState.squawk,
                aircraftState.spi, aircraftState.positionSource
            ];
            super(rawData);
        } else {
        ...
        this.speedKnots = this.velocity ? Math.round(this.velocity * 1.94384) : null;
        this.lastSeen = new Date(this.lastContact * 1000);

        // Flight trail (limited to 20 positions)
        this.trail = [];
        if (this.hasValidPosition()) {
            this.trail.push(new Position3D(
                this.longitude,
                this.latitude,
                altitude
            ));
    ...
    updateState(newState) {
        ...
        const oldPosition = this.hasValidPosition() ?
            new Position3D(this.longitude, this.latitude, oldAltitude) : null;
    ...
    getFormattedSpeed() {
        ...
        return `${this.speedKnots}kts (${Math.round(this.velocity)}m/s)`;
    }
     ...
     */
    isActive(maxAgeSeconds = 300) { // 5 minutes default
        const now = Date.now();
...
export class AirspaceViolation {
    ...
    getMessage() {
        ...
        return `Aircraft ${aircraftId} ${this.violationType.toLowerCase()} restricted airspace ${this.airspaceData.name}`;
    }
}
