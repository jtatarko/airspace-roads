...
export class AircraftVisualizer {
    ...
    updateAircraft(aircraftList) {
        ...
        console.log(`Aircraft update: +${added} ~${updated} -${removed} (${this.lastRenderTime.toFixed(1)}ms)`);
    }
    ...
    calculateOrientation(aircraft) {
        ...
        return Transforms.headingPitchRollQuaternion(
            Cartesian3.fromDegrees(aircraft.longitude, aircraft.latitude, aircraft.getAltitude() || 0),
            { heading, pitch, roll }
        );
    }
    ...
    shouldShowAircraft(aircraft) {
        ...
        // Type filter
        if (!this.enabledTypes.includes('all') &&
        ...
        if (aircraft.altitudeFeet !== null) {
            if (aircraft.altitudeFeet < this.altitudeFilter.min ||
        ...
        if (aircraft.speedKnots !== null) {
            if (aircraft.speedKnots < this.speedFilter.min ||
    ...
    getStatistics() {
        ...
        const visibleCount = Array.from(this.entities.values())
            .filter(entity => entity.show !== false).length;
