...
export class AirspaceUIControls {
  ...
  createAltitudeSlider() {
    ...
    container.innerHTML = `
            <div class="control-header">
                ...
                <span class="altitude-value">${this.formatAltitudeDisplay(
                  this.currentAltitude
                )}</span>
            </div>
            <input type="range" id="altitudeSlider" 
                   min="0" max="20000" step="100" value="${
                     this.currentAltitude
                   }">
  ...
  setupEventListeners() {
    if (this.altitudeSlider) {
      this.altitudeSlider.addEventListener("input", (e) => {
        ...
        this.altitudeValue.textContent = this.formatAltitudeDisplay(
          this.currentAltitude
        );
  ...
  showAirspaceInfo(airspace) {
    ...
    const classification = AirspaceClassifier.getClassificationInfo(
      airspace.icaoClass
    );
    ...
    const infoHTML = `
            <div class="airspace-details">
                ...
                <div class="detail-section">
                    ...
                    <br>
                    Lower: ${AirspaceDataProcessor.formatAltitude(
                      airspace.lowerAltitude,
                      airspace.isLowerAGL
                    )}
                    <br>
                    Upper: ${AirspaceDataProcessor.formatAltitude(
                  ...
                  Object.values(airspace.restrictions).some((r) => r)
                    ...
                    <div class="detail-section">
                        ...
                        ${Object.entries(airspace.restrictions)
                          ...
                          .map(
                            ([key, value]) =>
                              `<br>• ${this.formatRestrictionName(key)}`
                          )
                          .join("")}
                    </div>
                `
                ...
                <div class="detail-actions">
                    <button onclick="airspaceControls.focusOnAirspace('${
                      airspace.id
                    }')">
                        Focus on Airspace
                    </button>
